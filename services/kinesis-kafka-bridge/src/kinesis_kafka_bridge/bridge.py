...
def latest_iterator(client: Any, stream_name: str, shard_id: str) -> str:
    ...
    return str(resp["ShardIterator"])


def poll_shard(
    client: Any, shard_iterator: str
...
def republish(producer: Any, topic: str, records: list[dict[str, Any]]) -> int:
    ...
    for record in records:
        producer.send(
            topic, key=record["PartitionKey"].encode("utf-8"), value=record["Data"]
        )
    if records:
        producer.flush()
    return len(records)
