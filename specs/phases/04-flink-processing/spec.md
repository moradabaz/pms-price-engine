...
- **One MiniCluster-based smoke test** (`tests/test_minicluster_smoke.py`), scoped to Stage B only, not Stage A+B: runs Stage B's real `connect()`/`keyBy` wiring against an embedded MiniCluster, asserting the fan-out shape (AC-03/AC-04) end-to-end. Stage A is deliberately left out — Flink documents no ordering guarantee between a broadcast stream and its keyed counterpart, which would make a Stage A MiniCluster test either flaky or need to control internal timing the production job doesn't; that ordering-sensitivity is already covered at the component level (`test_stage_cost_enrichment.py`'s `test_no_emission_before_segment_assignment_arrives`). Stage B's own two-input join has the same theoretical ordering risk, sidestepped by construction: each `(apartment, night)` pair is fanned out exactly once, on whichever side of the pair arrives *second* — an invariant that holds for any interleaving, so the test asserts the exact M×N cross product without needing to control arrival order. `env.from_collection` only accepts pickle-simple types (a Java-side decoder, not Python's own) — plain strings go in, `.map()` builds the actual `CostAggregate`/`MarketPrice` objects, mirroring the same split `KafkaSource` + `.map()` already used in `job.py`.
- **Everything else (AC-08 checkpoint/restart, AC-11 live CDC flow) stays manual/live-verified**, matching Phases 2–3's precedent — a MiniCluster does not exercise real RocksDB-on-S3 checkpointing or a real Debezium connector, so simulating them would test the simulation, not the real failure modes those ACs care about. AC-11 was confirmed 2026-07-30: inserted a new row into `apartment_market_segments`, confirmed a matching message on `apartment-market-segments.v1` (offset advanced 10→11), and its fields matched the inserted row exactly.
