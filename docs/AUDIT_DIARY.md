...
**Status:** Implemented, not yet verified live against a running cluster. **Spec:** [`specs/phases/04-flink-processing/spec.md`](../specs/phases/04-flink-processing/spec.md). **Pre-spec:** [`docs/phase-4-streaming-design-decisions.md`](phase-4-streaming-design-decisions.md) (decisions A–H). **Diagram:** [`diagrams/phase-4-flink-processing.md`](../diagrams/phase-4-flink-processing.md). Branch `phase-4-flink-processing`, not yet merged, PR not yet opened.
...
**Non-obvious findings, all in [`error-handling/flink-operational-checklist.md`](../error-handling/flink-operational-checklist.md) — read before touching `streaming/flink-jobs/` again:**
1. A dead-man's-switch timer bug caught only by testing, not review: an early version compared full-precision `datetime` deadlines against Flink's millisecond-resolution timers — the equality check would have silently never matched, so the watchdog would fire on schedule but never actually flag anything stale.
2. PyFlink's `SinkFunction` only wraps existing Java sinks — there is no Python-native custom `Sink` class. The DynamoDB writer is implemented as a `MapFunction` (the write as a side effect) terminated by a Java `DiscardingSink`, confirmed against the actual installed `apache-flink==2.3.0`, not assumed.
3. PyFlink 2.3's Kinesis connector is the legacy `FlinkKinesisConsumer` only — no modern `KinesisSource` counterpart to Kafka's `KafkaSource`, confirmed the same way.

**Verified so far:** the pure business logic (pricing formula, replay-safety guard, state eviction, watchdog scan-on-fire) has full unit-test coverage (28 tests, `uv run --package flink-jobs pytest streaming/flink-jobs/tests`) and needs no Flink runtime to check. Stage A/Stage B's wiring was exercised against an in-memory `MapState`/`BroadcastState`/`TimerService` double (`tests/fakes.py`), catching the timer bug above. **Not yet verified:** anything requiring a real cluster — `KafkaSource`/`FlinkKinesisConsumer` need actual connector JARs on the classpath to even construct (confirmed by trying it directly: fails immediately with a clear "Java class not found" error without them), and wiring `apartment_market_segments` into `infra/debezium/postgres-connector.json`'s `table.include.list` (spec §4) hasn't been done yet either — this phase's own implementation work, not something Phase 1 or this session's work covered.
...
As of 2026-07-29: Phase 2 is merged to `main` (PR #4), Phase 3 is merged to `main` (PR #5, `phase-3-market-ingestion` — the branch-per-phase convention was restored, resolving the deviation this section used to flag). Phase 4 is implemented on its own branch, `phase-4-flink-processing`, not yet merged, PR not yet opened.

1. **Immediate next step: open the PR for `phase-4-flink-processing`.** Everything in the Phase 4 section above is committed to that branch — domain decisions (ADR-0006, ADR-0007), the formal spec, `apartment_market_segments` + its self-healing migration, the `market-ingestor` D.1/D.2 follow-up, and the `streaming/flink-jobs/` implementation itself.
2. **Before this can run live: wire `apartment_market_segments` into `infra/debezium/postgres-connector.json`'s `table.include.list`** (spec §4) — the table and its seed script exist, but nothing captures it via CDC yet. Follow the same verification discipline as Phase 2's four connector-config incidents (below) — don't assume the default topic name, key, or encoding are correct without checking live.
3. **Getting `streaming/flink-jobs/` to actually execute needs real connector JARs and a running cluster** — confirmed by trying to construct a `KafkaSource`/`FlinkKinesisConsumer` without them: fails immediately with a clear Java-class-not-found error. Not yet attempted with `pipeline.jars` configured against the actual `infra/docker-compose.yml` stack.
4. **Read `error-handling/` before modifying `infra/debezium/postgres-connector.json` or `streaming/flink-jobs/` again** — four connector-config gotchas from Phase 2, plus `flink-operational-checklist.md`'s Flink/PyFlink-specific findings from Phase 4 (max parallelism, Broadcast State ordering, per-key timers, PyFlink's out-of-process Python UDFs, RocksDB serializer compatibility, checkpoint alignment, the Kinesis connector gap, why State TTL isn't used, and a real timer-precision bug caught only by testing) — re-discovering any of them would be pure wasted time.
5. **Local environment reminders:** Docker Desktop must be running; `uv sync` for Python deps; `psql`/`kcat`/AWS CLI for manual verification per the README; the Maven build step (Phase 2) is required once per environment to reproduce the custom date converter. For Phase 4's pure logic: `uv run --package flink-jobs pytest streaming/flink-jobs/tests` needs no Docker/cluster at all.
6. **No automated integration tests exist for the CDC pipeline or for market ingestion** (Phase 2 spec §2, Phase 3 spec §2 — both deliberate scope decisions); Phase 4 is a partial exception — its pure business logic has full unit-test coverage and a component-level test against in-memory Flink-state doubles (spec §13), but nothing exercises a real cluster yet. If a future phase's timeline allows it, automating at least a subset of these phases' acceptance criteria (topic/stream message counts, schema validation, partition-affinity checks) would catch a regression far faster than the manual process used so far.
